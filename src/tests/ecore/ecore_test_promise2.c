...
_promise_ctx_new(void)
{
   ...
   fail_if(!ctx);
   ctx->p = eina_promise_new(efl_loop_future_scheduler_get(NULL), _cancel, ctx);
   fail_if(!ctx->p);
   return ctx;
}
...
_future_promise_create(void *data, const Eina_Value v EINA_UNUSED, const Eina_Future *dead_future EINA_UNUSED)
{
   ...
   p = eina_promise_new(efl_loop_future_scheduler_get(NULL), _inner_promise_cancel, NULL);
   fail_if(!p);
   ...
   for (i = 0; i < 3; i++)
     {
        ...
        p = eina_promise_new(efl_loop_future_scheduler_get(NULL), _promise_cancel_test, &cancel_called);
        fail_if(!p);
