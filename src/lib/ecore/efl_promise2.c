...
static Eina_Bool
_eina_value_equal(const Eina_Value v1, const Eina_Value v2)
{
   if (v1.type != v2.type)
     return EINA_FALSE;
...
_efl_future2_dispatch(Efl_Future2 *f, Eina_Value value)
{
    ...
    next_value = _efl_future2_dispatch_internal(&f, value);
    if (!_eina_value_equal(next_value, value)) _eina_value_safe_flush(value);
    if (!f)
      {
        ...
        if (f->cb)
          {
             ...
             if (!_eina_value_equal(value, r)) _eina_value_safe_flush(r);
          }
        f = _efl_future2_free(f);
     }
   _eina_value_safe_flush(value);
...
static void
_efl_future2_deliver(Efl_Promise2 *p,
                     Eina_Value value)
{
   if (p->future)
     {
...
_future_proxy(void *data, const Eina_Value v) {
   ...
   if (EINA_UNLIKELY(eina_log_domain_level_check(_promise2_log_dom, EINA_LOG_LEVEL_DBG)))
     {
        ...
        DBG("Future proxy called - scheduling promise: %p, Value Type: %s, Content: %s",
            p,
            eina_value_type_name_get(eina_value_type_get(&v)), str);
        free(str);
     }
   ...
   if (!_eina_value_equal(v, r)) _eina_value_safe_flush(r);
   eina_mempool_free(_promise_mp, p);
   return v;
}
...
efl_promise2_resolve(Efl_Promise2 *p, Eina_Value value)
{
   ...
   if (EINA_UNLIKELY(eina_log_domain_level_check(_promise2_log_dom, EINA_LOG_LEVEL_DBG)))
     {
        char *str = eina_value_to_string(&value);
        DBG("Resolve promise %p - Value Type: %s Content: %s", p,
            eina_value_type_name_get(eina_value_type_get(&value)), str);
        free(str);
     }
   _efl_future2_deliver(p, value);
}
